...
void main() {
  group('LeavePolicyRepository Tests', () {
    ...
    test('제출 성공 - 입사일 기준 (leaveAccrualBasis: 1)', () async {
      ...
      final result = await repository.submit(_makeRequest());

      expect(result, isA<Success<void, NetworkError>>());
    });

    test('제출 성공 - 회계연도 기준 (leaveAccrualBasis: 2, fiscalYearBaseMonth 포함)', () async {
      mockDioHelper.mockPost(
        path: path,
        responseData: {'success': true},
        statusCode: 200,
      ...
      final result = await repository.submit(
        _makeRequest(leaveAccrualBasis: 2, fiscalYearBaseMonth: 1),
      );

      expect(result, isA<Success<void, NetworkError>>());
    });

    test('제출 성공 - 201 응답', () async {
      mockDioHelper.mockPost(
        path: path,
        responseData: {'success': true},
        statusCode: 201,
